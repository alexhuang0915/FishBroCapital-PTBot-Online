...
export function extractStrategyName(filename) {
  const match = filename.match(/(MNQ|TXF)_(VIX|DX)_\d+/);
  if (match) {
    return match[0];
  }
...
export function loadAllStrategies(basePath, searchPaths = []) {
  const strategies = {};
  const strategyFiles = [
    { file: 'CME.MNQ HOT  MNQ_DX_60 策略回測績效報告.csv', name: 'MNQ_DX_60', currency: 'USD' },
    { file: 'CME.MNQ HOT  MNQ_VIX_120 策略回測績效報告.csv', name: 'MNQ_VIX_120', currency: 'USD' },
    { file: 'CME.MNQ HOT  MNQ_VIX_60 策略回測績效報告.csv', name: 'MNQ_VIX_60', currency: 'USD' },
    { file: 'TWF.TXF HOT  TXF_DX_60 策略回測績效報告.csv', name: 'TXF_DX_60', currency: 'TWD' },
    { file: 'TWF.TXF HOT  TXF_VIX_120 策略回測績效報告.csv', name: 'TXF_VIX_120', currency: 'TWD' },
    { file: 'TWF.TXF HOT  TXF_VIX_60 策略回測績效報告.csv', name: 'TXF_VIX_60', currency: 'TWD' },
  ];
  
  // Combine basePath with searchPaths
  const allPaths = [basePath, ...searchPaths];
  
  strategyFiles.forEach(({ file, name, currency }) => {
    let filePath = null;
    let useCSV = false;
    ...
    for (const searchPath of allPaths) {
      ...
      const excelPath = path.join(searchPath, file.replace('.csv', '.xlsx'));
