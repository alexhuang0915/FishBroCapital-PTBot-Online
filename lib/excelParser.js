...
export function loadAllStrategies(basePath, searchPaths = []) {
  const strategies = {};
  const strategyFiles = [
    { file: 'CME.MNQ HOT  MNQ_DX_60 策略回測績效報告.csv', excelFile: 'CME.MNQ HOT  MNQ_DX_60 策略回測績效報告.xlsx', name: 'MNQ_DX_60', currency: 'USD' },
    { file: 'CME.MNQ HOT  MNQ_VIX_120 策略回測績效報告.csv', excelFile: 'CME.MNQ HOT  MNQ_VIX_120 策略回測績效報告.xlsx', name: 'MNQ_VIX_120', currency: 'USD' },
    { file: 'CME.MNQ HOT  MNQ_VIX_60 策略回測績效報告.csv', excelFile: 'CME.MNQ HOT  MNQ_VIX_60 策略回測績效報告.xlsx', name: 'MNQ_VIX_60', currency: 'USD' },
    { file: 'TWF.MXF HOT  TXF_DX_60 策略回測績效報告.csv', excelFile: 'TWF.MXF HOT  MXF_DX_60 策略回測績效報告.xlsx', name: 'MXF_DX_60', currency: 'TWD' },
    { file: 'TWF.MXF HOT  TXF_VIX_120 策略回測績效報告.csv', excelFile: 'TWF.MXF HOT  MXF_VIX_120 策略回測績效報告.xlsx', name: 'MXF_VIX_120', currency: 'TWD' },
    { file: 'TWF.MXF HOT  TXF_VIX_60 策略回測績效報告.csv', excelFile: 'TWF.MXF HOT  MXF_VIX_60 策略回測績效報告.xlsx', name: 'MXF_VIX_60', currency: 'TWD' },
  ];
  
  // Combine basePath with searchPaths
  const allPaths = [basePath, ...searchPaths];
  
  strategyFiles.forEach(({ file, excelFile, name, currency }) => {
    let filePath = null;
    let useCSV = false;
    
    // Search in all paths
    for (const searchPath of allPaths) {
      // First try Excel file (preferred)
      ...
      // Then try CSV file
      const csvPath = path.join(searchPath, file);
      if (fs.existsSync(csvPath)) {
        filePath = csvPath;
        useCSV = true;
        break;
      }
      // Also try the old pattern (TXF -> MXF)
      const oldExcelPath = path.join(searchPath, file.replace('.csv', '.xlsx'));
      if (fs.existsSync(oldExcelPath)) {
