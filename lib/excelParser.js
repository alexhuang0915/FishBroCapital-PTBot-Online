...
export function convertTradesToDailyData(tradeData, strategyName, currency = 'USD') {
  ...
  if (firstRow) {
    ...
    if (isHeaderRow) {
      ...
      // CSV format: Column names are preserved (e.g., '日期', '獲利(¤)')
      // Excel format: '交易明細', '__EMPTY', '__EMPTY_1', '__EMPTY_3' (日期), '__EMPTY_7' (獲利)
      for (const [key, value] of Object.entries(firstRow)) {
        const strValue = String(value || '').trim();
        const strKey = String(key || '').trim();
        
        // Check both key and value for date column
        if (!dateCol && (strValue.includes('日期') || strKey.includes('日期'))) {
          dateCol = key;
          console.log(`  Found date column: ${key} = "${strValue}"`);
        }
        
        // Check both key and value for PnL column (獲利, 損益, PnL, Profit)
        if (!pnlCol && (
          strValue.includes('獲利') || strValue.includes('損益') || 
          strValue.includes('PnL') || strValue.includes('Profit') ||
          strKey.includes('獲利') || strKey.includes('損益') ||
          strKey.includes('PnL') || strKey.includes('Profit')
        )) {
          pnlCol = key;
...
export function loadAllStrategies(basePath, searchPaths = []) {
  ...
  const strategyConfigs = [
    // MNQ Strategies (USD)
    {
      ...
      csvFiles: [
        'CME.MNQ HOT  MNQ_DX_120_BackTest 策略回測績效報告_CSV.csv',
    ...
    },
    {
      ...
      csvFiles: [
        'CME.MNQ HOT  MNQ_VIX_60_BackTest 策略回測績效報告_CSV.csv',
    ...
    },
    {
      ...
      csvFiles: [
        'CME.MNQ HOT  MNQ_VIX_120_BackTest 策略回測績效報告_CSV.csv',
        'CME.MNQ HOT  MNQ_VIX_120 策略回測績效報告.csv'
      ]
    },
    {
      name: 'MNQ_ZN_120',
      ...
      csvFiles: [
        'CME.MNQ HOT  MNQ_ZN_120_BackTest 策略回測績效報告_CSV.csv',
    ...
    },
    {
      ...
      csvFiles: [
        'CME.MNQ HOT  MNQ_6J_60_BackTest 策略回測績效報告_CSV.csv',
        'CME.MNQ HOT  MNQ_6J_60 策略回測績效報告.csv'
      ]
    },
    {
      name: 'MNQ_6J_120',
      ...
      csvFiles: [
        'CME.MNQ HOT  MNQ_6J_120_BackTest 策略回測績效報告_CSV.csv',
        'CME.MNQ HOT  MNQ_6J_120 策略回測績效報告.csv'
      ]
    },
    // MXF Strategies (TWD)
    {
      ...
      csvFiles: [
        'TWF.MXF HOT  MXF_VIX_120_BackTest 策略回測績效報告_CSV.csv',
    ...
    },
    {
      ...
      csvFiles: [
        'TWF.MXF HOT  MXF_VIX_60_BackTest 策略回測績效報告_CSV.csv',
        'TWF.MXF HOT  MXF_VIX_60 策略回測績效報告.csv'
      ]
    },
    {
      name: 'MXF_ZN_120',
      ...
      csvFiles: [
        'TWF.MXF HOT  MXF_ZN_120_BackTest 策略回測績效報告_CSV.csv',
        'TWF.MXF HOT  MXF_ZN_120 策略回測績效報告.csv'
      ]
    },
    {
      name: 'MXF_6J_60',
      ...
      csvFiles: [
        'TWF.MXF HOT  MXF_6J_60_BackTest 策略回測績效報告_CSV.csv',
        'TWF.MXF HOT  MXF_6J_60 策略回測績效報告.csv'
      ]
    },
    {
      name: 'MXF_DX_60',
      ...
      csvFiles: [
        'TWF.MXF HOT  MXF_DX_60_BackTest 策略回測績效報告_CSV.csv',
        'TWF.MXF HOT  MXF_DX_60 策略回測績效報告.csv'
      ]
    },
    {
      name: 'MXF_DX_120',
      ...
      csvFiles: [
        'TWF.MXF HOT  MXF_DX_120_BackTest 策略回測績效報告_CSV.csv',
        'TWF.MXF HOT  MXF_DX_120 策略回測績效報告.csv'
      ]
    }
  ];
  ...
  // Combine basePath with searchPaths and additional paths
  const allPaths = [
    basePath, 
    ...searchPaths,
    strategyReportPath,
